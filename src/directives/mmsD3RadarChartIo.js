 ...
 angular.module('mms.directives')
    ...
    function mmsD3RadarChartIo(ElementService, UtilsService, $compile, growl, $window, mmsViewCtrl) {
      var mmsRadarChartLink = function(scope, element, attrs) {
        ...
        function createFilter(LegendOptions){
          //create only one filter display
          d3.selectAll('.graphFilter.'+ 'default').remove();
          var graphFilter = d3.select('div.'+'default').append('div')
              .attr("class", 'graphFilter '+ 'default')
              .style('margin-left', '10px');
          var filterLegendsDiv = graphFilter.append('div')
              .append('label').style('border', '1px solid #ddd')
              .text ("Filter by Legends");
          filterLegendsDiv.selectAll("div")
          .data(LegendOptions)
          .enter()  
        .append("div")
        //.attr("class", function(d,i){return "legentFilter "+ data.id + "-" + i;} )
        .attr("style", function(d,i){return "opacity: 1" + ";background-color:" + colorscale(i) + ";";})
        .on('mouseover', function (d, i){
            //var mouseoverId = (data.id+"-") + (i % data.legends.length);
              //  mouseover(mouseoverId);
            })
          ...
          .each(function (d, i) {
                ...
                d3.select(this).append("input")
                  ...
                  .on("click", function (d) {
                      ...
                      //d3.selectAll(".ghbbar."+ data.id + "-" + i ).style("visibility", this.checked ? "visible": "hidden");
                   });
